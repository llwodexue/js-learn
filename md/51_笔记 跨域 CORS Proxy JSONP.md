...
- fetch方法执行结果是一个promise实例（**不管当前请求成功与否，promise状态都是成功**的，**仅当网络故障或请求被阻止（CORS），promise状态才会是失败**的）

  `window.navigator.online`，来判断网络是否在线
...
- 客户端代码
  ...
  在客户端，有一些请求（ajax, fetch）受同源策略限制，就是只能在同域才能请求

  在客户端，有一些请求是非同源策略，就是可以在不同域发送请求，浏览器本身可以往任何地址发送请求、link、script、img、audio、video... 都可以往任何地址发送请求（不受源的限制）
