...
真实项目开发，我们一般把 link 放在页面的头部（是为了在没有渲染 DOM 的时候，就通知 HTTP 去请求 CSS 了，这样 DOM 渲染完，CSS 也差不多回来了，更有效的利用时间，提高页面的渲染速度）。我们一般把 JS 放在页面的底部，防止其阻碍 GUI 渲染，如果不放在底部，我们最好设置上 async/defer
