...
| 第 7 版     | 2016 年     | 幂运算符、数组扩展（includes）                               |
| 第 8 版     | 2017 年     | Async/await、字符串扩展、Object 静态方法                     |
...
console.log(obj) // { animal: 'dog', bark: [Function: bark] }
```
...
console.log(iter.next()) // 444 { value: undefined, done: true }
```
...
// Child() // TypeError: Class constructor Child cannot be invoked without 'new'。ES6 中创建的就是类，不能当做普通函数执行，只能 new 执行
...
ES9 允许命名捕获分组使用符号 `?<name>` ，将捕获结果赋值给具名分组，这样获取捕获结果可读性更强
...
- `Symbol.prototype.description` 是 ES10 新增的实例属性，在此之前读取 Symbol 描述需要通过 `Symbol.keyFor()` 或手动 `toString()` 解析

```js
let s1 = Symbol('bird')
console.log(s1.description) // bird

let s2 = Symbol.for('dog')
console.log(s2.description) // dog
```

> 关于 Symbol 的创建和 `Symbol.for()` 的详细用法，参见上方 [ES6 Symbol 章节](#10-symbol)
...
- Ruby 语言使用 `@` 表示私有属性，ECMAScript 没有用这个符号而使用 `#`，是因为 `@` 已经被留给了 Decorator
...
- 数值的精度只能到 53 个二进制位（相当于 16 个十进制位），超出这个范围的整数，JavaScript 无法精确表示
